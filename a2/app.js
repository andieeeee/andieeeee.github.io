...
function setupDrawing0() {
    ...
    function draw(p) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            // A stroke (outline) around the circle will be slightly darker
            // than the main color (pct*100 - 20)
            p.strokeWeight(1)
            p.stroke(hue, 100, pct*100)
            ...
            // let y = pct*HEIGHT
            let y = (.5 + .5*Math.sin(i*1 + t*.5))*p.height
...
function setupDrawing2() {
    ...
    function draw(p) {
        ...
        let angle = Math.sin(t*10)
        p.rotate(angle)

        let scale = 2*Math.sin(t*1) + 3
        p.scale(scale, scale)
...
function setupDrawing7() {
    ...
    function draw(p) {
        ...
        p.push()
        p.translate(p.width/20, p.height/20)
...
function setupDrawing8() {
    ...
    function draw(p) {
        ...
        p.push()
        p.translate(p.width/15, p.height/15);
        ...
        for (var i = 0; i < circlect; i++) {
            let hue = (loopPct*360 + i*10)%360
            let pastel = Math.sin(loopPct*Math.PI*2 + i*2)

            p.stroke(hue,100, 80, .8)
            p.fill(hue,100,50 + 50*pastel, .7 - i*.005)
            let x = i/circlect*p.width
            let y = (p.noise(100*i)+.005)*p.height
            p.circle(x, y, 75 + 20*Math.sin(loopPct*Math.PI*2 + i))
        }
